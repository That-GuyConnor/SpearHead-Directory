...
wepStats.chamberType = 1 -- Use 2 for guns that need to have their bolt opened when reloading, use 1 for normal guns, use 0 for rocket launchers.
wepStats.reloadType = 1 -- 1 = Magazines, 2 = Bullet insertion, 3 = Bullet insertion and clips

--< Gun settings >--
wepStats.fireRate = 625
wepStats.muzzleChance = 5 -- Number from 0-10 that determines how often the muzzle will flash when firing
wepStats.muzzleVelocity = 900 -- this stat uses meters per second
...
wepStats.maxPushback = 0.8 -- How far can the viewmodel move backwards until the gun is blocked
...
wepStats.fireMode = 2 -- Default mode from the above table
...
-- Tracers
wepStats.tracers = true
wepStats.tracerTiming = 4 -- Every x number of shots will be a tracer
wepStats.tracerColor = Color3.fromRGB(255, 55, 55)

-- Ammo
wepStats.ammoType = "5.45x39mm" -- Gun shell models can be found in ReplicatedStorage > SPH_Assets > Shells
wepStats.shellEject = true -- Should this gun eject shells?
wepStats.magazineCapacity = 30 -- Max ammo that can go in a mag
wepStats.magazineAmmo = 10 -- How much should start in the mag
wepStats.arcadeAmmo = true -- Don't disable this until the new ammo system is added
wepStats.startAmmoPool = 120 -- How much ammo should the gun start with?
wepStats.maxAmmoPool = 120 -- How much ammo can this gun hold?
...
-- Animation
wepStats.idleAnim = "Rifle_Idle" -- Animations are located in ReplicatedStorage > SPH_Assets > Animations
wepStats.sprintAnim = "Rifle_Sprint"
wepStats.reloadAnim = "AK_Reload"
wepStats.boltChamber = "AK_Chamber" -- Plays if the bolt is closed
wepStats.boltClose = "AK_Close" -- Plays if the bolt is open
wepStats.equipAnim = "Rifle_Equip"
wepStats.patrolAnim = "Rifle_Sprint"
wepStats.holdUpAnim = "Rifle_Neutral"
wepStats.holdDownAnim = nil
wepStats.switchAnim = "AK_Switch" -- Plays when switching fire mode

wepStats.reloadSpeedModifier = 0.8 -- 1 = Normal speed, higher = faster, lower = slower

wepStats.rigParts = {"Mag","Bolt","ChargingHandle"} -- These parts will have their welds replaced with Motor6Ds in case they need to be animated
wepStats.fireMoveParts = {"Bolt"} -- These parts will move when firing

wepStats.boltDist = 0.7 -- Distance the bolt will move when firing (Make this number negative for open bolt guns!)
...
wepStats.autoChamber = true -- Chamber the gun after reloading if needed
...
wepStats.recoil = {
	...
	damping = 1.5, -- How "springy" should the recoil be
...
wepStats.gunRecoil = {
	...
	punchMultiplier = 0.8, -- How much the gun moves back during recoil
}

wepStats.teamKill = true
