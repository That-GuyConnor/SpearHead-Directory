...
Operation Types 2 and 3 require a boltOpen animation.
]]

wepStats.openBolt = true
...
-- Ammo
wepStats.ammoType = ".38 Special" -- Gun shell models can be found in ReplicatedStorage > SPH_Assets > Shells
wepStats.shellEject = false -- Should this gun eject shells?
wepStats.magazineCapacity = 7 -- Max ammo that can go in a mag
wepStats.arcadeAmmo = true -- Don't disable this until the new ammo system is added
wepStats.startAmmoPool = 30 -- How much ammo should the gun start with?
wepStats.maxAmmoPool = 30 -- How much ammo can this gun hold?

wepStats.infiniteAmmo = true
...
-- Viewmodel
wepStats.viewmodelOffset = CFrame.new(0.4,0.1,0.5) -- Where should the viewmodel be placed in reference to the camera
...
wepStats.reloadAnim = "Revolver_Insert"
wepStats.boltChamber = "Pistol_Chamber" -- Plays if the bolt is closed
wepStats.boltOpen = "Revolver_Open"
wepStats.boltClose = "Revolver_Close" -- Plays if the bolt is open
...
wepStats.rigParts = {"CraneBase"} -- These parts will have their welds replaced with Motor6Ds in case they need to be animated
wepStats.fireMoveParts = {} -- These parts will move when firing
wepStats.bulletHolder = "CraneBase" -- If this gun has bullets that need to change visibility, this is the name of the thing they're inside.
...
-- Camera recoil
wepStats.recoil = {
	vertical = 3, -- Vertical recoil
	horizontal = 3, -- Horizontal recoil
